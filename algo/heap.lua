...
---@class algo.heap.node:table
---@field index number?

---@generic T:algo.heap.node
---@class algo.heap<T>
---@field data algo.heap.node[]
...
local function heap_new(is_left_above)
    ...
    local function heap_push(_, value)
        count = count + 1
        data[count] = value
        value.index = count
    ...
    local function heap_update_top(_)
        heap_update_index_down(1)
    end

    local function heap_update(_, value)
        heap_update_index(value.index)
    end

    local function heap_remove_top(_)
        if count == 0 then
            return
        end
        data[1].index = -1
    ...
    local function heap_remove(_, value)
    ...
    local function heap_top(_)
        return data[1]
    end

    local function heap_count(_)
        return count
    end
